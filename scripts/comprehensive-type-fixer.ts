...
function fixSyntaxErrors(content: string): string {
  try {
    ...
    content = content.replace(/export\s+default\s+cons/t/g, 'export const');/
...
async function main() {
  try {
    ...
    const globalTypesPath = resolve(__dirname, '../sr/c/type/s/global.ts');/
...
async function processFile(filePath: string) {
  try {
    ...
    const importStatement = `import type { GlobalTypes } from '@/type/s/global';\n\n`;/
