        ...
        <form className='mt-8 space-y-6' onSubmit={handleAdminPromotion}>
          <div className='rounded-md shadow-sm -space-y-px'>
            <div>
              ...
              <input id='invite-code' name='invite-code' type='text' required className='appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-primary-500 focus:border-primary-500 focus:z-10 sm:text-sm' placeholder='Enter Admin Invite Code' value={inviteCode} onChange={(e) => setInviteCode(e.target.value)} disabled={isLoading}/>
            </div>
          </div>

          {error && <div className='text-red-500 text-sm text-center'>{error}</div>}

          {success && (<div className='text-green-500 text-sm text-center'>
