...
class AuthService {
    ...
    private initializeFirebase() {
        try {
            ...
            throw new AuthServiceError('Firebase initialization failed', AuthErrorType.FIREBASE_INIT_ERROR, error as Error);
        }
    }
    ...
    // Sign up method
    public async signUp(email: string, password: string, displayName?: string, role: UserRole = USER_ROLES.USER): Promise<UserProfile> {
        this.validateFirebaseInitialized();
        this.validateAuthInput(email, password);
        try {
    ...
    async sendPasswordResetCode(email: string, resetCode?: string): Promise<boolean> {
        ...
        try {
            ...
            throw new AuthServiceError('Password reset failed', this.mapFirebaseAuthError(error), error as Error);
        }
    }
    ...
    async updateProfile(payload: UpdateProfilePayload): Promise<UserProfile> {
        try {
            ...
            const authError = error instanceof AxiosError
                ? new AuthServiceError(error.response?.data?.message || 'Profile update failed', AuthErrorType.UNAUTHORIZED, error)
...
export { AuthErrorType, AuthServiceError, AuthService };
