    ...
    <div className='bg-neutralBackground-100 shadow-soft rounded-xl p-4 sm:p-6'>
      <div className='flex flex-col sm:flex-row items-center mb-4 sm:mb-6'>
        <img
          ...
          alt='Profile'
          className='w-12 h-12 sm:w-16 sm:h-16 rounded-full object-cover mb-2 sm:mb-0 sm:mr-4 border-4 border-primary-100'
        />
      ...
      <div className='grid grid-cols-2 md:grid-cols-3 gap-2 sm:gap-4 mb-4 sm:mb-6'>
        {Object.entries(activityCounts || {}).map(([type, count]) => (
          <div
            key={type}
            className='bg-neutralBackground-50 p-2 sm:p-4 rounded-md text-center border border-neutralBackground-500'
          >
            ...
            {sortedActivities.map((activity, index) => (
              <li key={index} className='py-2 sm:py-4'>
                <div className='flex justify-between items-center'>
                  ...
                  {activity.details && (
                    <span className='text-xs sm:text-sm text-accent1-500'>{activity.details}</span>
                  )}
                </div>
              </li>
            ))}
