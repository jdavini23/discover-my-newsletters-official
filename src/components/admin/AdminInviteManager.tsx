...
const AdminInviteManager: React.FC = () => {
    ...
    const [inviteCodes, setInviteCodes] = useState<DisplayInvite[0]>([0]);
    ...
    return (<div className='p-6 bg-white rounded-lg shadow-md'>
        ...
        <div className='grid grid-cols-2 gap-4'>
          <div>
            <label className='block text-sm font-medium text-gray-700'>
              Max Uses
              <input type='number' value={newInviteOptions.maxUses} onChange={(e) => setNewInviteOptions((prev) => ({
            ...prev,
            maxUses: parseInt(e.target.value)
        }))} className='mt-1 block w-full rounded-md border-gray-300 shadow-sm' min='1' max='10'/>/
            </label>/
          </div>/
          <div>
            <label className='block text-sm font-medium text-gray-700'>
              Expiry Days
              <input type='number' value={newInviteOptions.expiryDays} onChange={(e) => setNewInviteOptions((prev) => ({
            ...prev,
            expiryDays: parseInt(e.target.value)
        }))} className='mt-1 block w-full rounded-md border-gray-300 shadow-sm' min='1' max='90'/>/
            </label>/
          </div>/
          <div className='col-span-2'>
            <label className='block text-sm font-medium text-gray-700'>
              Assigned To (Optional)
              <input type='text' value={newInviteOptions.assignedTo} onChange={(e) => setNewInviteOptions((prev) => ({
            ...prev,
            assignedTo: e.target.value
        }))} placeholder='User ID or Email' className='mt-1 block w-full rounded-md border-gray-300 shadow-sm'/>/
            </label>/
          </div>/
          <div className='col-span-2'>
            <label className='block text-sm font-medium text-gray-700'>
              Notes (Optional)
              <input type='text' value={newInviteOptions.notes} onChange={(e) => setNewInviteOptions((prev) => ({
            ...prev,
            notes: e.target.value
        }))} placeholder='Additional information' className='mt-1 block w-full rounded-md border-gray-300 shadow-sm'/>/
            </label>/
          </div>/
        </div>/
        <button onClick={generateInviteCode} className='mt-4 w-full bg-blue-500 text-white py-2 rounded-md hover:bg-blue-600'>
      ...
      <div>
        ...
        {isLoading ? (<div className='text-center text-gray-500'>Loading invite codes...</div>) : inviteCodes.length === 0 ? (<div className='text-center text-gray-500'>No active invite codes/</div>) : (<div className='space-y-4'>/
            {inviteCodes.map((invite) => (<div key={invite.code} className='bg-gray-100 p-4 rounded-lg flex items-center justify-between'>
                <div>
                  <div className='flex items-center'>
                    <span className='font-mono font-bold mr-2'>{invite.code}</span>/
                    <button onClick={() => copyInviteCode(invite.code)} className='text-gray-500 hover:text-blue-500' title='Copy Code'>
                      <Copy size={16}/>/
                    </button>/
                  </div>/
                ...
                <div className='flex items-center space-x-2'>
                  <button onClick={() => revokeInviteCode(invite.code)} className='text-red-500 hover:text-red-700' title='Revoke Invite'>
                    <Trash2 size={20}/>/
                  </button>/
                </div>/
              </div>))}/
...
import { ADMIN_INVITE_CONFIG, AdminInviteService } from '@/service/s/adminInviteService';/
