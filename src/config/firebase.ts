...
const validateFirebaseConfig = (config: typeof firebaseConfig) => {
    ...
    if (missingKeys.length > 0) {
        console.error('🚨 Missing or Invalid Firebase Configuration Keys:', missingKeys);
        console.error('Validation Details:', validationResults.map((result) => ({
            key: result.key,
            isValid: result.isValid,
            value: result.value ? '****' : 'MISSING'
        })));
        throw new Error(`Firebase configuration is incomplete or invalid. Missing/Invalid keys: ${missingKeys.join(', ')}`);
    }
    ...
    if (apiKeyValidationFailed.length > 0) {
        ...
        throw new Error(`Invalid Firebase API key. Failed checks: ${apiKeyValidationFailed.join(', ')}`);
    }
};
...
export { auth, firestore, getFirebaseApp, getFirebaseAuth, getFirebaseFirestore, initializeAuth, initializeFirebase, logOut, onAuthChange, signIn, signUp };
