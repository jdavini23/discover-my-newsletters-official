...
function validateFirebaseCredentials() {
  console.log('ðŸ” Firebase Credentials Validation Tool ðŸ”');
  console.log('');
  ...
  if (!isValid) {
    console.error('\nâŒ Invalid Firebase Credentials Configuration');
  ...
  if (
    ...
    console.error('\nâŒ Invalid Private Key Format');
  ...
  try {
    ...
    console.error('\nâŒ Failed to initialize Firebase Admin:', error);
    process.exit(1);
  }
}
